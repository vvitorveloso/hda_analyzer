...
class Monitor(gtk.Window):
  ...
  def __init__(self, parent=None, device=None):
     ...
     hbox.set_border_width(4)
     for idx in [2, 4, 6, 8]:
        button = gtk.Button(f"{idx} channels")
     ...
     self.record_p = None
     self.set_title(f'ALSA Monitor for {self.device}')
     self.show_all()
  ...
  def __channel_change(self, button, idx):
     if self.channel != idx or self.generate_p is None:
  ...
  def __channels_change(self, button, idx):
     if self.channels != idx or self.record_p is None:
  ...
  def generate_sound(self):
     self.set_status(
         f'Playing sound #{self.generate_num} on channel {CHANNELS[self.channel]}...'
     )
     self.generate_num += 1
     channels = 2
     if self.channel >= 6:
  ...
  def generate_cleanup(self):
     if not self.generate_p:
       return
     if self.generate_p.poll() is None:
        try:
  ...
  def record_sound(self):
     self.set_status(f'Recording sound - {self.channels} channels...')
